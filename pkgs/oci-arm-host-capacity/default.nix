{ php, php81Packages, curl, stdenv, git, source, lib,writeShellScriptBin, ... }:

stdenv.mkDerivation (source.oci-arm-host-capacity // rec {
...
'';
  ...
  installPhase =
let
  wrapped = writeShellScriptBin "run" ''
    exec ${php}/bin/php $out/index.php
  '';
  in
    ''
    composer install
