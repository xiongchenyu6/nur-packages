...
in {
  ...
  config = let
    ...
    DELIVERY_ETH_RPC_URL =
      if cfg.mainnet
      then "https://mainnet.infura.io/v3/${cfg.infuraKey}"
      else "https://goerli.infura.io/v3/${cfg.infuraKey}";
    BSC_RPC_URL =
      if cfg.mainnet
      then "https://bsc-dataseed.binance.org/"
      else "https://data-seed-prebsc-1-s1.binance.org:8545/";
    TRON_RPC_URL =
      if cfg.mainnet
      then "grpc.trongrid.io:50051"
      else "47.252.19.181:50051";
    TRON_GRID_URL =
      if cfg.mainnet
      then "https://tronevent.bt.io/"
      else "https://test-tronevent.bt.io";
    ...
    mkIf cfg.enable {
      ...
      systemd.services = {
        ...
        deliveryd = {
          ...
          environment = {DELIVERY_HOME_DIR = "${cfg.deliveryHomeDir}";};
          preStart =
            ''
              set -x
              ...
              ${update_toml "prometheus"
                (
                  if cfg.prometheus
                  then "true"
                  else "false"
                )
                "$DELIVERY_HOME_DIR/config/config.toml"}
            ...
            + (
              if cfg.deiliveryPrivateKeyPath != null
              then ''
