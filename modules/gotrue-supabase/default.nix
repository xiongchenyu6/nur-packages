...
let
  ...
  formatValue = v:
    if builtins.isBool v then boolToString v else if builtins.isInt v then toString v else v;

  environment =
    mapAttrs (_: formatValue) (filterAttrs (_: v: v != null) (mkMerge [
      {
...
in
{
  ...
  config = mkIf cfg.enable {
    warnings =
      ...
      ++ (optional (cfg.environmentFiles == [ ] && cfg.apiExternalUrl == null && !(cfg.settings ? API_EXTERNAL_URL))
        "services.gotrue-supabase.apiExternalUrl unset; set apiExternalUrl, settings.API_EXTERNAL_URL, or use an environment file.")
      ++ (optional (cfg.environmentFiles == [ ] && cfg.databaseUrl == null && !(cfg.settings ? DB_DATABASE_URL))
        "services.gotrue-supabase.databaseUrl unset; set databaseUrl, settings.DB_DATABASE_URL, or use an environment file.")
      ++ (optional (cfg.environmentFiles == [ ] && cfg.jwtSecret == null && !(cfg.settings ? GOTRUE_JWT_SECRET))
