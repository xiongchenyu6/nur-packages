# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  combobulate = {
    pname = "combobulate";
    version = "17c71802eed2df1a6b25199784806da6763fb90c";
    src = fetchgit {
      url = "https://github.com/mickeynp/combobulate.git";
      rev = "17c71802eed2df1a6b25199784806da6763fb90c";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-m+06WLfHkdlMkLzP+fah3YN3rHG0H8t/iWEDSrct25E=";
    };
    date = "2025-06-21";
  };
  copilot-el = {
    pname = "copilot-el";
    version = "4f51b3c21c42756d09ee17011201ea7d6e18ff69";
    src = fetchgit {
      url = "https://github.com/zerolfx/copilot.el.git";
      rev = "4f51b3c21c42756d09ee17011201ea7d6e18ff69";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-5nB+skW0yjKvLUkDcwFJGSsCcRPZnJ44KoiTJno2iRM=";
    };
    date = "2025-06-30";
  };
  gotron-sdk = {
    pname = "gotron-sdk";
    version = "v0.24.1";
    src = fetchFromGitHub {
      owner = "fbsobreira";
      repo = "gotron-sdk";
      rev = "v0.24.1";
      fetchSubmodules = false;
      sha256 = "sha256-FkalIfUWUX5BaIBYaqh+i3S+5mvaShfRAUL+UYMymdo=";
    };
  };
  gptel = {
    pname = "gptel";
    version = "7bb353a582d11ce210bb0a5b77c53de564c28e67";
    src = fetchgit {
      url = "https://github.com/karthink/gptel.git";
      rev = "7bb353a582d11ce210bb0a5b77c53de564c28e67";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-5Zn+hFuEWi/lJA9/lvIyBrJsXm7pDjbGZjnb21TLC38=";
    };
    date = "2025-08-23";
  };
  helmify = {
    pname = "helmify";
    version = "v0.4.18";
    src = fetchFromGitHub {
      owner = "arttor";
      repo = "helmify";
      rev = "v0.4.18";
      fetchSubmodules = false;
      sha256 = "sha256-yk+ST0W7HLlAe5NOaBrQvpG6AYgwWDkpK5/ml5TLAQw=";
    };
  };
  korb = {
    pname = "korb";
    version = "v2.3.4";
    src = fetchFromGitHub {
      owner = "BeryJu";
      repo = "korb";
      rev = "v2.3.4";
      fetchSubmodules = false;
      sha256 = "sha256-jznXK+YlPjglafrTA1jlCuxKkZSetgk9GeY0FWxnHy4=";
    };
  };
  librime-lua = {
    pname = "librime-lua";
    version = "68f9c364a2d25a04c7d4794981d7c796b05ab627";
    src = fetchgit {
      url = "https://github.com/hchunhui/librime-lua.git";
      rev = "68f9c364a2d25a04c7d4794981d7c796b05ab627";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-m7/qXdIlMMHscDDcFmusNuOR0cuzPpDQdprqRci8qZw=";
    };
    date = "2025-07-07";
  };
  magit-gitflow = {
    pname = "magit-gitflow";
    version = "cc41b561ec6eea947fe9a176349fb4f771ed865b";
    src = fetchgit {
      url = "https://github.com/jtatarik/magit-gitflow.git";
      rev = "cc41b561ec6eea947fe9a176349fb4f771ed865b";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-LqBvn+Ev8GsZG6zmQ5eGt+5vDQk5ywqvagrvnzNP5ks=";
    };
    date = "2017-09-29";
  };
  magit-town = {
    pname = "magit-town";
    version = "4ce962e6e2d8249d3a5aa4c40e9924a424b50ebf";
    src = fetchgit {
      url = "https://github.com/natecox/magit-town.git";
      rev = "4ce962e6e2d8249d3a5aa4c40e9924a424b50ebf";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-dcLarzN9miAxzW9IP3IECxjRuShR/D3m5eU7vV6YE+A=";
    };
    date = "2019-04-02";
  };
  my2sql = {
    pname = "my2sql";
    version = "1ff5d20e854fd7be43ff07c88358fe3754f29af0";
    src = fetchurl {
      url = "https://github.com/liuhr/my2sql/raw/master/releases/centOS_release_7.x/my2sql";
      sha256 = "sha256-ggsA5U6AgszBuwAipX8BSuQSA0o67GDnkTZNkHhPpfo=";
    };
    date = "2022-11-27";
  };
  netbird = {
    pname = "netbird";
    version = "v0.55.1";
    src = fetchFromGitHub {
      owner = "netbirdio";
      repo = "netbird";
      rev = "v0.55.1";
      fetchSubmodules = false;
      sha256 = "sha256-Bi5vHxKdl9eWWdTB7td+rHuvjt6Ic4kjytWzHJTpCtQ=";
    };
  };
  org-cv = {
    pname = "org-cv";
    version = "e8de952df7669b38ca475d00fe943ab96d8cfac4";
    src = fetchgit {
      url = "https://gitlab.com/Titan-C/org-cv.git";
      rev = "e8de952df7669b38ca475d00fe943ab96d8cfac4";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-qwJTtyfNZpA4YCOxh90hXW89aYThE9K6GnJIxeUI7No=";
    };
    date = "2025-01-24";
  };
  sui = {
    pname = "sui";
    version = "mainnet-v1.54.2";
    src = fetchurl {
      url = "https://github.com/MystenLabs/sui/releases/download/mainnet-v1.54.2/sui-mainnet-v1.54.2-ubuntu-x86_64.tgz";
      sha256 = "sha256-Xs3LC62wBVZEFsAER8vJi1kTGkMwG5+TnVU3NWuOvfE=";
    };
  };
}
